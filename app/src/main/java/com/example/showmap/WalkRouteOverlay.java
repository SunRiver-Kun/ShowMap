...
public class WalkRouteOverlay extends RouteOverlay {
	...
	public WalkRouteOverlay(Context context, AMap amap, WalkPath path,
		...
		this.mAMap = amap;
		this.walkPath = path;
		startPoint = com.example.showmap.AMapServicesUtil.convertToLatLng(start);
		endPoint = com.example.showmap.AMapServicesUtil.convertToLatLng(end);
	}
	/**
	 * 添加步行路线到地图中。
	 * @since V2.1.0
    ...
    public void addToMap() {
        ...
        try {
            ...
            for (int i = 0; i < walkPaths.size(); i++) {
                ...
                LatLng latLng = com.example.showmap.AMapServicesUtil.convertToLatLng(walkStep
                        .getPolyline().get(0));
    ...
    private void addWalkPolyLine(LatLonPoint pointFrom, LatLonPoint pointTo) {
        addWalkPolyLine(com.example.showmap.AMapServicesUtil.convertToLatLng(pointFrom), com.example.showmap.AMapServicesUtil.convertToLatLng(pointTo));
    }
    ...
    private void addWalkPolyLines(WalkStep walkStep) {
        mPolylineOptions.addAll(com.example.showmap.AMapServicesUtil.convertArrList(walkStep.getPolyline()));
    }
